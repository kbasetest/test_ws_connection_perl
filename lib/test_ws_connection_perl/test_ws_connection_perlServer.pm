package test_ws_connection_perl::test_ws_connection_perlServer;
...
sub get_service_name
{
    ...
    if(!defined $ENV{$SERVICE}) {
        return 'test_ws_connection_perl';
    }
    return $ENV{$SERVICE};
}
...
sub call_method {
    ...
    my $ctx = test_ws_connection_perl::test_ws_connection_perlServerContext->new($self->{loggers}->{userlog},
...
{
    # Service test_ws_connection_perl requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for test_ws_connection_perl but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = test_ws_connection_perl::test_ws_connection_perlServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service test_ws_connection_perl.");
    }
...
package test_ws_connection_perl::test_ws_connection_perlServerContext;

use strict;

=head1 NAME

test_ws_connection_perl::test_ws_connection_perlServerContext
...
package test_ws_connection_perl::test_ws_connection_perlServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = test_ws_connection_perl::test_ws_connection_perlServer->new(
