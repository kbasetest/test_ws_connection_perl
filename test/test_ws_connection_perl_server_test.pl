...
my $config = new Config::Simple($config_file)->get_block('test_ws_connection_perl');
...
$test_ws_connection_perl::test_ws_connection_perlServer::CallContext = $ctx;
my $impl = new test_ws_connection_perl::test_ws_connection_perlImpl();

sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_test_ws_connection_perl_' . $suffix;
...
eval {
    ...
    my $ret = $impl->run_test_ws_connection_perl({workspace_name => get_ws_name(),
...

{
    ...
    sub provenance {
        my($self) = @_;
        return [{'service' => 'test_ws_connection_perl', 'method' => 'please_never_use_it_in_production', 'method_params' => []}];
    }
    sub authenticated {
        return 1;
    }
